-- Yönetici görev yönetimi ve kanıt temizliği.
-- Görev oluşturma, atama, düzenleme ve silme yalnızca yöneticide kalır.
...
BEGIN
  IF current_user_id IS NULL OR NOT public.has_role(current_user_id, 'admin') THEN
    RAISE EXCEPTION 'Görev oluşturma yalnızca yönetici yetkisindedir';
  END IF;
...
BEGIN
  IF current_user_id IS NULL OR NOT public.has_role(current_user_id, 'admin') THEN
    RAISE EXCEPTION 'Görev düzenleme yalnızca yönetici yetkisindedir';
  END IF;
  ...
  IF previous_status = 'external_approval' AND new_status IS DISTINCT FROM previous_status THEN
    RAISE EXCEPTION 'Onay bekleyen görevin durumu yalnızca onay işlemiyle değişir';
  END IF;
  IF new_status IS DISTINCT FROM previous_status
     AND new_status NOT IN ('not_started', 'blocked', 'not_applicable') THEN
    RAISE EXCEPTION 'İlerleme ve tamamlanma durumu yalnızca kanıtlı onay akışıyla değişir';
  END IF;
  IF assigned_user_id IS NOT NULL
     AND NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = assigned_user_id) THEN
    RAISE EXCEPTION 'Atanacak kullanıcı bulunamadı';
...
BEGIN
  ...
  IF current_status = 'external_approval' AND new_status IS DISTINCT FROM current_status THEN
    RAISE EXCEPTION 'Onay bekleyen görevin durumu yalnızca onay işlemiyle değişir';
  END IF;
  IF new_status NOT IN ('not_started', 'blocked', 'not_applicable') AND new_status IS DISTINCT FROM current_status THEN
    RAISE EXCEPTION 'İlerleme ve tamamlanma durumu yalnızca kanıtlı onay akışıyla değişir';
  END IF;
...
CREATE OR REPLACE FUNCTION public.delete_operational_task(target_task_id UUID)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  IF (SELECT auth.uid()) IS NULL OR NOT public.has_role((SELECT auth.uid()), 'admin') THEN
    RAISE EXCEPTION 'Görev silme yalnızca yönetici yetkisindedir';
  END IF;
  DELETE FROM public.operational_tasks WHERE id = target_task_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'Görev bulunamadı'; END IF;
END;
$$;

CREATE OR REPLACE FUNCTION public.delete_extra_project_task(target_task_id UUID)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  ...
  END IF;
  IF EXISTS (
    SELECT 1 FROM public.project_task_progress_submissions
    WHERE project_task_id = target_task_id
  ) THEN
    RAISE EXCEPTION 'İlerleme geçmişi olan proje görevi silinemez; Uygulanmaz olarak işaretleyin';
  END IF;
  DELETE FROM public.project_tasks WHERE id = target_task_id AND phase_order = 999;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Yalnızca ilerleme geçmişi olmayan ek proje görevleri silinebilir';
  END IF;
END;
$$;

CREATE OR REPLACE FUNCTION public.delete_project_task_evidence(target_evidence_id UUID)
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  ...
  SELECT storage_path INTO target_path
  FROM public.project_task_evidence
  WHERE id = target_evidence_id
    AND submission_id IS NULL
    AND (uploaded_by = current_user_id OR public.has_role(current_user_id, 'admin'))
  FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Kanıt silinemez; onaya gönderilmiş olabilir veya yetkiniz yoktur';
  END IF;
  DELETE FROM public.project_task_evidence WHERE id = target_evidence_id;
  RETURN target_path;
END;
$$;

CREATE OR REPLACE FUNCTION public.delete_work_photo(target_photo_id UUID)
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  IF current_user_id IS NULL THEN RAISE EXCEPTION 'Oturum bulunamadı'; END IF;
  SELECT photo.storage_path INTO target_path
  FROM public.photos AS photo
  WHERE photo.id = target_photo_id
    AND (photo.uploaded_by = current_user_id OR public.has_role(current_user_id, 'admin'))
    AND NOT EXISTS (SELECT 1 FROM public.progress_updates AS progress WHERE progress.evidence_photo_id = photo.id)
    AND NOT EXISTS (SELECT 1 FROM public.work_completion_evidence AS completion WHERE completion.photo_id = photo.id)
  FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Fotoğraf silinemez; onaya gönderilmiş olabilir veya yetkiniz yoktur';
  END IF;
...
-- Taşeron fiziksel depolama dosyasını doğrudan silemez. Yanlış kanıt kaydı
-- ekrandan kaldırılır; fiziksel temizlik sadece yönetici tarafından yapılır.
DROP POLICY IF EXISTS "wp_contractor_delete_own" ON storage.objects;

REVOKE ALL ON FUNCTION public.update_operational_task(UUID, TEXT, TEXT, UUID, UUID, UUID, DATE, public.project_task_status) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.delete_operational_task(UUID) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.delete_extra_project_task(UUID) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.delete_project_task_evidence(UUID) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.delete_work_photo(UUID) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.update_operational_task(UUID, TEXT, TEXT, UUID, UUID, UUID, DATE, public.project_task_status) TO authenticated;
GRANT EXECUTE ON FUNCTION public.delete_operational_task(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.delete_extra_project_task(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.delete_project_task_evidence(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.delete_work_photo(UUID) TO authenticated;
