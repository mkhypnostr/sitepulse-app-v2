-- Kalıcı silme akışında activity_logs tetikleyicisinin silinmiş kayda tekrar
-- foreign key ile bağlanmasını engeller.
...
BEGIN
  -- Kalıcı silme fonksiyonları bağlı kayıtları da temizlerken yeni audit kaydı
  -- üretmez; böylece silinmiş bir kayda FK ile tekrar bağlanılmaz.
  ...
  IF TG_TABLE_NAME = 'projects' THEN
    -- AFTER DELETE aşamasında proje artık yoktur; log proje FK'siz saklanır.
  ...
  INSERT INTO public.activity_logs (
    project_id, work_order_id, entity_type, entity_id, action, actor_id, old_data, new_data
  ) VALUES (
    resolved_project_id,
...
BEGIN
  ...
  DELETE FROM public.activity_logs WHERE work_order_id = target_work_order_id;
  DELETE FROM public.work_orders WHERE id = target_work_order_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'Saha görevi bulunamadı'; END IF;
END;
$$;
